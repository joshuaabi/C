...
scanf("%d", &qntClientes);
	...
	for(i = 0; i < qntClientes; i++){												
		...
		scanf(" %[^\n]s", clientes[i].nameClientes);									/* Leitura de dados  inseridos pelo usuario */
		scanf("%5", &clientes[i].numbCliente);
    ...
    do{
		...
		switch(opcaoMenu) {
			...
			case 4:
				...
				printf("Listagem dos clientes em ordem alfabética.\n");
		...
		if(opcaoMenu != 0) {
			printf("Pressione qualquer tecla para continuar...");												/* Interação com o menu .   */
			getchar();
			getchar();
		}
...
/*
 ...
 //	EXEMPLO CRESCENTE
void selecaoCRESCENTE(int qntClientes, Clientes clientes[])

{

int i, j, min, aux;
   for (int i = 0; i < qntClientes-1; ++i) {
       min = i;
      
      for (int j = i+1; j < qntClientes; ++j){	  
         if (clientes[j] < clientes[min])  {
		 min = j;
		 aux = v[i];
		 v[i] = v[min]
		 v[min] = aux;
	...
	// Transferir uma string pra outra
	strcpy(destino, orgiem);
